# SshCloneHarness — smoke test del clone SSH (Nivel 2)
#
# Valida la hipótesis central ANTES de cablear nada en NestJS:
#   "git + openssh autentican y clonan por rama y por commit SHA de verdad"
#
# Uso:
#   docker build -t ssh-harness -f Dockerfile.harness .
#   docker run --rm \
#     -e REPO_URL="git@bitbucket.org:WORKSPACE/REPO.git" \
#     -e CLONE_REF="main" \
#     -e COMMIT_SHA="" \
#     -v /ruta/a/tu/deploy_key:/tmp/ssh_key:ro \
#     ssh-harness
#
# Si COMMIT_SHA viene vacío, clona por rama (CLONE_REF).
# Si COMMIT_SHA viene, hace `git checkout <COMMIT_SHA>` dentro del clon.
...
if (!repoUrl) fail("REPO_URL es obligatorio");
if (!fs.existsSync(keyPath)) fail(`No existe la llave en ${keyPath}`);
...
try {
  ...
  execFileSync(
    "git",
    ["clone", "--depth", "1", "--branch", cloneRef, repoUrl, cloneDir],
    { env: { ...process.env, GIT_SSH_COMMAND: sshCmd }, stdio: "inherit" },
  );
  ...
  if (files.length === 0) fail("El clone no devolvió archivos");
